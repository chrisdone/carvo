...
var alphabet = [a_, b_, c_, d_, e_, f_, g_, h_, i_, j_, k_, l_, m_, n_, o_, p_, q_, r_, s_, t_, u_, v_, w_, x_, y_];
...
function main(){
  ...
  if (canvas.getContext) {
    ...
    var chunks = poem.split(/ /);
    forloop(2000,0,function(){return chunks.length},1,function(i,done){
      letters(chunks[i],done);
    },function(){
    });
  }
}

function letters(string,done){
  var els = [];
  for (var i = 0, len = string.length; i < len; i++) {
    if (string[i] == '\n') {
      els.push([cr,lf]);
    } else {
      els.push(alphabet[string.charCodeAt(i) - 97]);
    }
  }
  turtle(ctx,state,[].concat.apply([],els),done);
}
...
function turtle(ctx,state,steps,done){
  ctx.clearRect(0,0,dimensions.width,dimensions.height);
  state = newstate();
  // lines(ctx,dimensions.width,dimensions.height);
...
function cr(){
  state.x = verticalLines.lead*dimensions.width;
  return true;
}
...
function draw(ctx,state){
  ctx.beginPath();
  ctx.arc(state.x,state.y,0.4 + state.pressure,0,Math.PI*2);
  ctx.fill();
}
...
"               shall be lifted—nevermore!"].join(" ").replace(/[^a-z ]/,'');
